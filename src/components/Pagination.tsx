...
export const FeedPagination = () => {
	...
	const {isLoading, totalArticles}: IarticleState = useSelector((state: AppStateType) => state.articles)
	const [localPage, setLocalPage] = useState(1)
	const {sorting, searchQuery, category}: IqueryState = useSelector((state: AppStateType) => state.query)
 ...
 useEffect(() => {
	...
	return(
		<>
			{!isLoading && <Pagination count={countPageHandler()} page={localPage} onChange={handleChange} />}
		</>
		)
}