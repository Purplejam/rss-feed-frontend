...
export const ArticleSmall = ({content, guid}: IArticleSmallState) => {
	...
	const handleChangeSubmit = async (newContent: string) => {
		const url = 'http://localhost:5000/api/v1/feed/update-article'
	...
	const handleRemoveSubmit = async() => {
		handleClose()
		const url = 'http://localhost:5000/api/v1/feed/remove-article'
		const options = {
			withCredentials: true,
			headers: {'Content-Type': 'application/json'},
			params: {
 ...
 const handleClose = () => {
	...
	return(
		<ArticleSmallStyle>
			...
			<div className="article-icons">
				<button className="delete-button" onClick={handleOpen}>{trashIcon}</button>
   ...
   {openModal && <Modal
     ...
     <DeleteModal>
       ...
       <span></span>
       <div>          
         <button onClick={handleRemoveSubmit} className="simple-button inverse">Так</button>
         <button onClick={handleOpen} className="simple-button inverse">Ні</button>
       </div>
     </DeleteModal>
    </Modal>}
		</ArticleSmallStyle>
